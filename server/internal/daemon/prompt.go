...
func buildCommentPrompt(task Task, provider string) string {
	...
	fmt.Fprintf(&b, "1. Run `rtk multica issue get %s --output json` to understand your task, then decide how to proceed.\n", task.IssueID)
	fmt.Fprintf(&b, "2. Read the triggering thread first: `rtk multica issue comment list %s --thread %s --tail 30 --output json` returns the root + the 30 most recent replies in that thread (root is always included, even at `--tail 0`, so you keep the \"what is this about\" context without dragging hundreds of replies into your prompt). If 30 replies aren't enough, walk older replies in the same thread one page at a time by passing the stderr `Next reply cursor: --before <ts> --before-id <reply-id>` line back as `--before <ts> --before-id <reply-id>` on the next call. If you also need cross-thread background, `rtk multica issue comment list %s --recent 20 --output json` pulls the 20 most recently active threads on the issue; under `--recent` the same `--before` / `--before-id` flags walk older *threads* (stderr label: `Next thread cursor`) instead of older replies. Avoid the unfiltered `--output json` form on long-running issues; it dumps the full flat timeline (cap 2000) and wastes context. `--since <RFC3339>` is still available for incremental polling and may combine with `--thread --tail` or `--recent`.\n", task.IssueID, task.TriggerCommentID, task.IssueID)
	b.WriteString("3. If the work requested was already completed in a previous session, do NOT redo it — reply with a concise summary of what was done.\n")
	b.WriteString("4. Only do new or additional work if the comment explicitly asks for something that has not already been completed.\n")
	b.WriteString("5. ")
