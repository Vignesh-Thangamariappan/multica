...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	if ctx.ChatSessionID != "" {
		...
		fmt.Fprintf(&b, "2. Run `rtk multica issue comment list %s --output json` to read the conversation (returns all comments, capped server-side at 2000)\n", ctx.IssueID)
		...
		if ctx.IsSquadLeader {
			...
			fmt.Fprintf(&b, "5. **Post your final results as a comment — this step is mandatory**: `rtk multica issue comment add %s --content \"...\"`. Your results are only visible to the user if posted via this CLI call; text in your terminal or run logs is NOT delivered.\n", ctx.IssueID)
		}
		fmt.Fprintf(&b, "6. When done, run `rtk multica issue status %s in_review`\n", ctx.IssueID)
		fmt.Fprintf(&b, "7. If blocked, run `rtk multica issue status %s blocked` and post a comment explaining why\n\n", ctx.IssueID)
	}
